...
import {
  Plus, Search, FileText, Star, Pin, Trash2, Archive,
  Tag, MoreHorizontal, X, Save,
} from "lucide-react";
...
export function Notes() {
  ...
  const createNote = useMutation({ mutationFn: api.createNote, onSuccess: () => qc.invalidateQueries({ queryKey: ["notes"] }) });
  ...
  const colorCls = COLORS.find((c) => c.key === (selected?.color || editColor))?.cls || "";
        ...
        <div className="border-b border-white/[0.06] p-4">
          <div className="mb-3 flex items-center justify-between">
            ...
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={handleNew}
              className="flex items-center gap-1.5 rounded-lg bg-white/[0.07] px-3 py-1.5 text-xs font-medium text-white/70 hover:bg-white/10 hover:text-white transition-all">
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              {!search && <button onClick={handleNew} className="mt-3 text-xs text-white/40 hover:text-white/70 underline">Erste Notiz erstellen</button>}
            </div>
          ) : (
